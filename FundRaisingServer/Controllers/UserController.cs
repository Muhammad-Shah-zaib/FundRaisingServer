...
public class UserController(IUserRepository userRepo, IUserAuthLogRepository userAuthLogRepo, IPasswordRepository passwordRepo, IUserTypeRepository userTypeRepo) : ControllerBase
{
    ...
    public async Task<ActionResult<UserResponseDto>> UpdateUser([FromRoute] int userCnic, UserUpdateRequestDto userUpdateRequestDto)
    {
        ...
        try
        {
            ...
            if (user == null) return NotFound($"User# {userCnic} not found.");
    
            // now we need to check if the updated email exist or not
            ...
            // now add an updated Log
            if (!await this._userAuthLogRepo.SaveUserAuthLogAsync(userUpdateRequestDto.UserId, UserEventType.Updated)) return StatusCode(500, "Internal server error");
            // now we can update the userType
            // since it is possible the user don't want to update the userType
            // So we need to check it first
            if (userUpdateRequestDto.UserType != null && userUpdateRequestDto.UserType != user.UserTypes.Select(u => u.Type).FirstOrDefault())
